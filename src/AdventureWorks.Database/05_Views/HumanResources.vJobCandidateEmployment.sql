...
IF EXISTS
    (
        SELECT
            1
        FROM
            fn_listextendedproperty('MS_Description', 'schema', 'HumanResources', 'view', 'vJobCandidateEmployment', DEFAULT, DEFAULT)
    )
BEGIN
    EXEC sys.sp_dropextendedproperty @name = N'MS_Description',
                                     ...
                                     @level1type = N'view',
                                     @level1name = 'vJobCandidateEmployment';
END
GO

EXEC sys.sp_addextendedproperty @name = N'MS_Description',
                                @value = N'Displays the content from each employement history related element in the xml column Resume in the HumanResources.JobCandidate table. The content has been localized into French, Simplified Chinese and Thai. Some data may not display correctly unless supplemental language support is installed.',
                                @level0type = N'schema',
                                @level0name = 'HumanResources',
                                @level1type = N'view',
                                @level1name = 'vJobCandidateEmployment';
